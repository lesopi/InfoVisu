class TwoDThreeD {
  ...
  float [][] physicalCorners = {//4x4
    // TODO:
    // Store here the 3D coordinates of the corners of
    // the real Lego board, in homogenous coordinates
    // and clockwise.
    {-boardSize/2, -boardSize/2, 0, 1}, //A
    {boardSize/2, -boardSize/2, 0, 1}, //B
    {boardSize/2, boardSize/2, 0, 1}, //C
    {-boardSize/2, +boardSize/2, 0, 1}//D
  };
  ...
  double[][] solveExtrinsicMatrix(List<PVector> points2D) {
    ...
    for (int i=0; i<4; i++) {
      ...
      float[] homo ={points2D.get(i).x,points2D.get(i).y,1};
      projectedCorners[i] = Mat.multiply(invK, homo);//matrix 3x3    
